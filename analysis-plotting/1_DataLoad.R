...
#########################################################
###  Export for FRAM-eDNA paper (Metfies et al)
#########################################################
...
#########################################################
  ###  Export for polarDNAexplorer ShinyApp
#########################################################

ASV %>% write.table(
  file="../../polarDNAexplorer/data/CTD_asv.txt",
  sep="\t", row.names=T, col.names=T, quote=F)
TAX %>% write.table(
  file="../../polarDNAexplorer/data/CTD_tax.txt",
  sep="\t", row.names=T, col.names=T, quote=F)
...
# Sort correctly
ENV %>% 
  arrange(region, Station, date, layer) %>%
  dplyr::select(
    region, Station, date, layer, lat, lon, depth,
    temp, sal, iceConc, iceDist, O2_conc, O2_sat,
    #icePast,iceDistPast,lat,lon,depth,locus_tag,
    jday, year, Carbohydrates,`Amino acids`,`Sugar acids`, 
    Amines, DOC, TDN, chl, sample_title) %>%
  dplyr::rename(
    Region = region,
    Layer = layer,
    `Sea-ice cover` = iceConc,
    `Distance ice edge` = iceDist,
    ...
    `Julian day` = jday,
     ...
     Longitude = lon) %>% 
 write.table(
  file="../../polarDNAexplorer/data/CTD_meta.txt",
  sep="\t", row.names=F, col.names=T, quote=T)


#########################################################

## Set modified theme_classic
